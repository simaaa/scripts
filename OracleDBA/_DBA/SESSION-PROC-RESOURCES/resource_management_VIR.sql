...
select t.* from dba_rsrc_consumer_groups t where t.internal_use != 'YES' /*and t.consumer_group like 'VIR%'*/ order by t.consumer_group;
select t.* from dba_rsrc_consumer_group_privs t where t.granted_group like '%FHB%' order by t.granted_group, t.grantee;
select t.* from dba_rsrc_group_mappings t where t.consumer_group like '%FHB%' order by t.attribute, t.value, t.consumer_group;
...
select t.* from dba_rsrc_plan_directives t /*where t.plan like '%VIR%'*/ order by t.plan, t.group_or_subplan, t.type;
