-- Generating daily table partition splitter DDL commands if SPLITTABLE partition exists
DECLARE
  ...
  v_part_date_start      DATE             := DATE'2017-01-01'; -- Day of first partition
  v_part_date_end        DATE             := DATE'2019-01-01'; -- Day of last Partition
  v_split_part_name      VARCHAR2(100)    := 'D99991231';      -- Splittable partition name ('D99991231','D20181201')
  ...
  -- Daily calendar cursor
  CURSOR cur_calendar(i_StartDate IN DATE, i_EndDate IN DATE) IS
    SELECT (i_StartDate+rownum-1) AS DAY, 'D'||TO_CHAR(i_StartDate+rownum-1,'YYYYMMDD') AS PART_NAME FROM dual CONNECT BY rownum <= i_EndDate - i_StartDate;
...
BEGIN
  ...
  FOR cur IN cur_calendar(v_part_date_start,v_part_date_end) LOOP --DBMS_OUTPUT.PUT_LINE( TO_CHAR(cur.day,'YYYY.MM.DD') || ' - ' || cur.part_name );
    ...
    v_sql := 'ALTER TABLE ' || v_table_owner || '.' || v_table_name || ' SPLIT PARTITION ' || v_split_part_name || ' AT' || CASE WHEN MULTILINE THEN CHR(13) ELSE NULL END ||
             '       ( TO_DATE('' ' || TO_CHAR(cur.day+1,'YYYY-MM-DD HH24:MI:SS') || ''', ''SYYYY-MM-DD HH24:MI:SS'') )' || CASE WHEN MULTILINE THEN CHR(13) ELSE NULL END ||
